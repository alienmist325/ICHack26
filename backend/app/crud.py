...
def _row_to_property(row: sqlite3.Row) -> Property:
    ...
    json_fields = [
        'amenities', 'price_history', 'images', 'floorplans', 
        'brochures', 'epc', 'nearest_schools'
    ]
...
def get_property_by_rightmove_id(rightmove_id: str) -> Optional[Property]:
    ...
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM properties WHERE rightmove_id = ?", 
            (rightmove_id,)
        )
        row = cursor.fetchone()
...
def update_property(property_id: int, property_data: PropertyUpdate) -> Optional[Property]:
...
def soft_delete_property(property_id: int) -> bool:
    ...
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE properties SET removed = 1 WHERE id = ?",
            (property_id,)
        )
...
def get_properties_with_scores(
    ...
    """
    properties = get_properties(filters, limit=limit * 2, offset=offset)  # Get more to account for filtering
