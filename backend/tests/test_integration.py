...
def test_conversion():
    ...
    for prop in properties:
        ...
        assert converted.rightmove_id == prop.id
        assert converted.listing_title == prop.title
        assert converted.price == float(prop.price)
...
def test_storage():
    ...
    for prop in properties:
        ...
        assert stored_prop.id is not None
        assert stored_prop.rightmove_id == prop.id

    print(f"\n✓ Successfully stored {stored_count}/{len(properties)} properties")
    assert stored_count == len(properties)
...
def test_filtering():
    ...
    # Verify results
    assert len(all_props) > 0, "Should have retrieved some properties"
    assert len(price_filtered) >= 0, "Price filter should work"
    assert len(bedroom_filtered) >= 0, "Bedroom filter should work"
    assert len(type_filtered) >= 0, "Type filter should work"
...
def test_scoring():
    ...
    assert len(all_props) > 0, "Should have at least one property to score"

    for prop in all_props:
        scored_prop = get_property_with_score(prop.id)
        assert scored_prop is not None, f"Should score property {prop.id}"
        ...
        assert scored_prop.upvotes >= 0
        assert scored_prop.downvotes >= 0
        assert scored_prop.total_votes == scored_prop.upvotes + scored_prop.downvotes
